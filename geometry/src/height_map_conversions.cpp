...
#include "math/geometry/height_map_conversions.h"
#include <iostream>
...
VisualMesh height_map_to_visual_mesh(
    ...
    math::VisualMesh mesh;
    math::VisualMesh::Vertex vertex;
    ...
    math::Matrixd<2, 3> projection = math::Matrixd<2, 3>::Zero();
    projection.block<2, 2>(0, 0).setIdentity();

    math::Vector2d cell_size = size.array() / divisions.cast<double>().array();

    for (math::Vector2i index = math::Vector2i::Zero(); index != divisions; step_grid_index(index, divisions)) {
        math::Vector2d cell_centre;
        cell_centre = origin.array() + (index.cast<double>().array() + 0.5).array() * cell_size.array();

        math::Vector3d normal = height_map.normal(cell_centre);

        std::size_t vertices_offset = mesh.vertices.size();
        for (int k = 0; k < 4; k++) {
            math::Vector2i corner_index = index;
            if (k % 2 == 1) {
                corner_index(0)++;
            }
            if (k >= 2) {
            ...
            math::Vector2d corner_position =
                origin + math::Vector2d(corner_index.cast<double>().array() * cell_size.array());
...
HeightMapGrid height_map_to_height_map_grid(
    ...
    const math::Vector2i& divisions)
{
    math::Vector2i divisions_plus_one = divisions + math::Vector2i::Ones();
    ...
    for (
        math::Vector2i index = math::Vector2i::Zero();
        index != divisions_plus_one;
        step_grid_index(index, divisions_plus_one))
    {
        math::Vector2d point = origin + math::Vector2d(index.cast<double>().array() * cell_size.array());
        grid.get_height(index) = height_map.height(point);
        std::cout << "Point: " << point.transpose() << std::endl;
        std::cout << "Height: " << grid.get_height(index) << std::endl;
    }

    math::Vector2i i00(0, 0);
    math::Vector2i i01(0, 1);
    math::Vector2i i10(1, 0);
    math::Vector2i i11(1, 1);
    ...
    {
        math::Vector2d p00 = origin + math::Vector2d((index + i00).cast<double>().array() * cell_size.array());
        math::Vector2d p01 = origin + math::Vector2d((index + i01).cast<double>().array() * cell_size.array());
        math::Vector2d p10 = origin + math::Vector2d((index + i10).cast<double>().array() * cell_size.array());
        math::Vector2d p11 = origin + math::Vector2d((index + i11).cast<double>().array() * cell_size.array());
