...
#include "core/std/optional_tuple.h"
...
struct optional_manifold_iterator {
    static int dynamic_dim(const Tuple& tuple, int dim) {
        ...
        dim += math::manifold_dynamic_dim(tuple.template value<Index>());

        return optional_manifold_iterator<Tuple, Remaining-1>::dynamic_dim(tuple, dim);
    }
    static void add(Tuple& tuple, const math::VectorXd& delta, int dim) {
        static_assert(Remaining > 0);
        static constexpr int Index = Tuple::capacity() - Remaining;

        if (!tuple.template has_value<Index>()) return;
        int element_dim = math::manifold_dynamic_dim(tuple.template value<Index>());
        math::manifold_add(tuple.template value<Index>(), delta.block(dim, 0, element_dim, 1));
        dim += element_dim;

        optional_manifold_iterator<Tuple, Remaining-1>::add(tuple, delta, dim);
    }
    static math::VectorXd difference(const Tuple& a, const Tuple& b, const math::VectorXd& delta, int dim) {
        static_assert(Remaining > 0);
        static constexpr int Index = Tuple::capacity() - Remaining;
        ...
        int element_dim = math::manifold_dynamic_dim(b.template value<Index>());
        assert(math::manifold_dynamic_dim(b.template value<Index>()) == element_dim);

        delta.block(dim, 0, element_dim, 1) = math::manifold_difference(
...
struct optional_manifold_iterator<Tuple, 0> {
    ...
    math::VectorXd difference(const Tuple& a, const Tuple& b, const math::VectorXd& delta, int dim) {
        return delta;
    }
};

template <typename ...Args>
struct math::manifold_details<core::OptionalTuple<Args...>> {
    typedef core::OptionalTuple<Args...> X;
    ...
    static void add(X& x, const math::VectorXd& delta) {
        optional_manifold_iterator<X, X::capacity()>::add(x, delta, 0);
    }
    static math::VectorXd difference(const X& a, const X& b) {
        math::VectorXd delta = math::VectorXd::Zero(dynamic_dim(a));
...
struct tuple_manifold_iterator {
    ...
    static constexpr int element_dim = math::manifold_dim<element_t>;
    ...
    static int dynamic_dim(const Tuple& tuple, int current_dim)
    {
        ...
        if constexpr(element_dim == -1) {
            current_dim += math::manifold_dynamic_dim(std::get<Index>(tuple));
        }
        return tuple_manifold_iterator<Tuple, Remaining-1, new_prev_dim>::dynamic_dim(tuple, current_dim);
    }

    static void add(Tuple& tuple, const math::Vectord<dim>& delta, int delta_index)
    {
        static_assert(Remaining > 0);
        if constexpr(dim != -1) {
            math::manifold_add(std::get<Index>(tuple), delta.template block<element_dim, 1>(delta_index, 0));
            delta_index += element_dim;
        }
        if constexpr(dim == -1) {
            int dynamic_dim = math::manifold_dynamic_dim(std::get<Index>(tuple));
            math::manifold_add(std::get<Index>(tuple), delta.block(delta_index, 0, dynamic_dim, 1));
            delta_index += dynamic_dim;
        }
        tuple_manifold_iterator<Tuple, Remaining-1, new_prev_dim>::add(tuple, delta, delta_index);
    }

    static math::Vectord<dim> difference(const Tuple& a, const Tuple& b, const math::Vectord<dim>& delta, int delta_index)
    {
        static_assert(Remaining > 0);

        if constexpr(dim != -1) {
            delta.template block<element_dim, 1>(delta_index, 0) = math::manifold_difference(
                a.template value<Index>,
                b.template value<Index>
            );
            delta_index += element_dim;
        ...
        if constexpr(dim == -1) {
            int dynamic_dim_a = math::manifold_dynamic_dim(a.template value<Index>());
            int dynamic_dim_b = math::manifold_dynamic_dim(b.template value<Index>());
            assert(dynamic_dim_a == dynamic_dim_b);
            int dynamic_dim = dynamic_dim_a;

            delta.block(delta_index, 0, dynamic_dim, 1) = math::manifold_difference(
                a.template value<Index>,
                b.template value<Index>
            );
            delta_index += dynamic_dim;
...
struct tuple_manifold_iterator<Tuple, 0, PrevDim> {
    ...
    static void add(Tuple& tuple, const math::VectorXd& delta, int delta_index) {
        // Do nothing
    }
    math::VectorXd difference(const Tuple& a, const Tuple& b, const math::VectorXd& delta, int delta_index) {
        return delta;
    }
};

template <typename ...Args>
struct math::manifold_details<std::tuple<Args...>> {
    ...
    static void add(X& x, const math::Vectord<dim>& delta) {
        tuple_manifold_iterator<X, std::tuple_size_v<X>, 0>::add(x, delta, 0);
    }
    static math::Vectord<dim> difference(const X& a, const X& b) {
        math::Vectord<dim> delta = math::Vectord<dim>::Zero(dynamic_dim(a));
